...
- 新着レビュー: 各店5〜10件かつレビューURLの異なるドメインを2サイト以上必須とし、生成日から365日以内の公開日または訪問月・5点評価・要約・好評点・注意点を保存します。根拠ページが年月までしか表示しない場合は月初日として新着判定し、HTMLには年月だけを表示します。検索プロンプトには実日付の対象期間を渡し、未来日、期間外、重複レビュー、情報源が1サイトだけの候補は保存前に除外して理由を実行結果へ記録します。
...
Web調査の構造化出力が壊れたJSONになった場合や、重複・範囲外候補に偏って保存前のコード判定で5店に満たない場合は、デフォルトで最大3回まで追加調査して候補プールを合算します。試行回数を変える場合は`--research-attempts`を指定します。
