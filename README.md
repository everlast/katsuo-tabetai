...
OpenAI Agents SDK for Pythonを学ぶための、Web検索から候補保存、実handoff、決定論的評価、HTML出力までを1回のrunで行うサンプルです。

```text
Katsuo Research Agent
  ├─ WebSearchTool（店舗・カツオ料理・根拠URLを検索）
...
- 構造化保存: `outputs/restaurant_candidates.json`をPydanticモデルから保存します。
- 根拠URL: 全候補の`evidence_url`を必須にし、HTMLにも店舗ごとのリンクを表示します。
- 範囲判定: ホテルと店舗の緯度経度からHaversine式で直線距離を計算し、`within_range`をコードで決定します。
- 決定論スコア: 保存済みの事実だけから100点満点で計算し、同点時は距離、店舗名の順で整列します。
- HTML: `outputs/top5.html`へTOP 5を出力します。
...
```text
outputs/
├── restaurant_candidates.json  # 検索で得た全候補、距離、範囲判定、根拠URL
├── top5.json                    # 決定論的に採点した上位5店
└── top5.html                    # ブラウザで開けるランキング
```

候補JSONの主要フィールドは`name`, `address`, `latitude`, `longitude`, `katsuo_dish`, `evidence_url`, `evidence_source_type`, `source_urls`, `distance_km`, `within_range`です。

## 決定論スコア

同じ`restaurant_candidates.json`と距離上限を入力すれば、常に同じ結果になります。LLMは採点と並べ替えをしません。

- 根拠の種別: 最大35点（店舗公式35、観光公式29、予約22、レビュー14）
- カツオ料理の特徴: 最大28点（料理名10、藁焼き8、塩たたき5、旬の案内5）
- 独立した根拠URL: 最大12点（ドメイン単位、最大3件）
- ホテルからの距離: 最大25点（距離上限まで線形減点）
...
APIキーなしで、距離・範囲判定、スコアの再現性、TOP 5整列、HTML内の根拠URL、エージェント構成をテストできます。
