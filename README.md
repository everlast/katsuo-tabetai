...
- 構造化保存: 検証済み店舗を`outputs/restaurants/`へ店舗単位で即時保存し、5店揃った時点の候補プールを`outputs/restaurant_candidates.json`へ集約します。
- 根拠URL: 全候補の`evidence_url`とレビューごとの`review_url`を必須にし、HTMLにもリンクを表示します。
- 新着レビュー: 各店5〜10件かつレビューURLの異なるドメインを2サイト以上必須とし、生成日から365日以内の公開日または訪問月・5点評価・要約・好評点・注意点を保存します。根拠ページが年月までしか表示しない場合は月初日として新着判定し、HTMLには年月だけを表示します。検索プロンプトには実日付の対象期間を渡し、未来日、期間外、重複レビューを個別に除外します。除外後も5件・2サイト以上残る店舗は有効レビューだけで保存し、条件を下回る候補は除外理由を実行結果へ記録します。
...
Web調査の構造化出力が壊れたJSONになった場合や、重複・範囲外候補に偏って保存前のコード判定で5店に満たない場合は、デフォルトで最大3回まで追加調査して候補プールを合算します。各回で検証を通った店舗は、最終的に5店揃わず実行が失敗しても`outputs/restaurants/`へ残り、次回の候補プールに引き継がれます。試行回数を変える場合は`--research-attempts`を指定します。

## 出力

```text
outputs/
├── restaurants/               # 店舗ごとの再利用可能な候補・レビューJSON
...
`outputs/restaurants/`の各ファイルは、正規化した店名と住所から作る安定したキーで保存されます。同じ店舗を再調査すると更新され、住所の異なる支店は別ファイルになります。起動時にはレビュー期間、重複、5件以上、2サイト以上、現在のホテルからの距離をコードで再検証し、条件を満たす店舗だけを候補プールへ読み込みます。期限外・未来日・重複レビューを除いても5件以上残る場合は、店舗全体を捨てず有効レビューだけを再利用します。

個別ファイルがまだない状態で既存の`restaurant_candidates.json`がある場合は、最初の起動時に店舗単位のファイルへ自動移行します。個別キャッシュに古い店舗が残っていても、再検証を通らなければランキングには使われません。
...
レビューの「新着」は候補JSONの生成日から365日以内です。各店舗で最低2つの異なるレビューURLドメインを必須とし、1サイトだけの候補は保存しません。根拠ページが訪問年月しか表示しない場合、`published_at`にはその月の1日を保存して期間判定し、HTMLでは`YYYY-MM`までを表示します。採点時は保存済みレビューの5点評価、件数、URLドメイン数だけを使い、LLMに点数や順位を決めさせません。推薦理由も保存済みの料理特徴、レビュー集約、距離から定型ロジックで生成します。
