...
- 段階的なデータ収集: 口コミサイトの個別店舗・口コミページを起点に、検証可能な口コミを持つ候補の発見と即時保存を優先します。その後、店舗公式・観光公式ページで店名、住所、カツオ料理、料理特徴を補完します。起動時に収集済み候補が5店以上あり、評価可能店が5店未満なら、店舗発見を挟まず収集済み候補を5店ずつ選んで口コミを補完します。多数の店舗と全口コミを1応答へ詰め込んで口コミが欠落することを防ぎます。
...
Web調査の構造化出力が壊れた場合や、評価可能候補が5店に満たない場合は、デフォルトで初回を含め最大3回まで調査します。キャッシュ読込時点で収集済み候補が5店以上あれば、1回目から口コミ補完を実行します。各回で発見した範囲内店舗は評価条件を満たさなくても`outputs/restaurants/`へ残り、同じ店舗の追加レビューや根拠は次回以降の観測と重複排除しながら合算します。試行回数を変える場合は`--research-attempts`を指定します。
