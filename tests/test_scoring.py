...
def test_score_uses_configured_evidence_points() -> None:
    ...
    expected_points = {
        EvidenceSourceType.OFFICIAL_RESTAURANT: 25.0,
        EvidenceSourceType.OFFICIAL_TOURISM: 21.0,
        EvidenceSourceType.RESERVATION_SITE: 16.0,
        EvidenceSourceType.REVIEW_SITE: 10.0,
    }
...
def test_katsuo_feature_points_have_a_twenty_point_maximum() -> None:
    ...
    assert score_candidate(candidate, 2.5).katsuo_features == 8.0

    feature_points = {
        "has_warayaki": 5.0,
        "has_shio_tataki": 4.0,
        "has_seasonal_katsuo": 3.0,
    }
    for field, expected_increment in feature_points.items():
        featured = candidate.model_copy(update={field: True})
        assert (
            score_candidate(featured, 2.5).katsuo_features == 8.0 + expected_increment
        )

    all_features = candidate.model_copy(
        update={field: True for field in feature_points}
    )
    assert score_candidate(all_features, 2.5).katsuo_features == 20.0
